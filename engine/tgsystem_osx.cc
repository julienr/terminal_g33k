...
void tgSystemOSX::Initialize ()
{
	bDone = false;
	for (int i=0; i<TGKEY_LAST; i++) bKeyDown[i] = false;
}

tgIGame* tgSystemOSX::GetGameAPI (const char* name)
{
	pDLHandle = dlopen("./game.dylib", RTLD_LAZY);
	pGame = NULL;
...
void tgSystemOSX::CloseGameAPI ()
{
	deleteGame (pGame);
	dlclose (pDLHandle);
}

bool tgSystemOSX::OpenWindow (const char* name, int sizeX, int sizeY)
{
	iScreenWidth = sizeX;
	iScreenHeight = sizeY;
...
void tgSystemOSX::CloseWindow ()
{
	SDL_Quit ();
}

void tgSystemOSX::PreRender ()
{

}

void tgSystemOSX::PostRender ()
{
	SDL_GL_SwapBuffers();
}

void tgSystemOSX::ReceiveEvents ()
{
	SDL_Event event;
